...
public interface DietRepository extends JpaRepository<Diet, Long> {
	...
	@Query("SELECT d FROM Diet WHERE d.start >= :start ORDER BY d.start")
	Page<Diet> findByStart(Instant start);
	...
	@Query("SELECT d FROM Diet d WHERE d.start >= :date AND d.end <= :date ORDER BY d.start")
	Page<Diet> findByDate(Instant date);
}
