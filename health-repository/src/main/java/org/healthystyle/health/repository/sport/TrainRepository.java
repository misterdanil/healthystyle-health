...
public interface TrainRepository extends JpaRepository<Train, Long> {
	...
	Page<Train> findByDay(Integer day, Pageable pageable);
	...
	Page<Train> findPlanned();

	@Query(value = "WITH nextTrainTime AS ("
			+ "SELECT t.time FROM train t INNER JOIN sport s ON t.sport_id = s.id WHERE CURRENT_DATE BETWEEN s.start AND s.end AND t.day = extract(dow FROM CURRENT_DATE) AND t.time >= CURRENT_TIME ORDER BY m.time LIMIT 1)"
			+ ")"
			+ "SELECT t FROM train t INNER JOIN sport s ON t.sport_id = s.id WHERE CURRENT_DATE BETWEEN s.start AND s.end AND t.day = extract(dow FROM CURRENT_DATE) AND t.time == nextTrainTime")
	Page<Train> findNextTrain();
}
