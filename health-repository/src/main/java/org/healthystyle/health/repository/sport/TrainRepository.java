...
public interface TrainRepository extends JpaRepository<Train, Long> {
	...
	@Query(value = "SELECT t FROM train t INNER JOIN sport s ON s.id = t.sport_id WHERE s.health_id = :healthId AND s.end >= CAST(CURRENT_TIMESTAMP AS DATE) ORDER BY s.start, t.day >= (isodow FROM CURRENT_TIMESTAMP) DESC, t.day, t.time >= CURRENT_TIME DESC, t.time", nativeQuery = true)
